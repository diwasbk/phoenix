...
import { handleGetApplicationByID, handleSubmitApplication, handleUpdateApplicationDetailByID } from "@/app/lib/actions/application-actions";
...
export default function ApplicationForm({ applicationId }: { applicationId: string }) {
        ...
        const fetchApplicationDetail = async () => {
            try {
                ...
                reset({
                    ...
                    parentPhone: res.result.parentPhone,
                    responsiblePerson: res.result.responsiblePerson,
                    responsiblePhone: res.result.responsiblePhone,
                    foreignLanguage: res.result.foreignLanguage,
                    testPreparation: res.result.testPreparation,
                    otherService: res.result.otherService,
                    preferredCountry: res.result.preferredCountry,
                    referralSource: res.result.referralSource,
                    termsAgreed: res.result.termsAgreed
                })
    ...
    const onSubmit = async (data: applicationType) => {
        try {
            const res = await handleUpdateApplicationDetailByID(applicationId, data);
            ...
            <div className="rounded-3xl border border-blue-100 bg-white p-5 shadow-sm sm:p-6">
                ...
                <div className="mt-5 grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <div>
                        ...
                        <div className="mt-3 space-y-3 text-sm text-slate-700">
                            {['IELTS', 'SAT', 'JLPT / NAT', 'Other'].map((item) => (
                                <label key={item} className="flex items-center gap-3">
                                    <input
                ...
                <div className="mt-6 space-y-4">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <button
                            ...
                            {isSubmitting ? "Updating..." : "Update Application Form"}
                        </button>
                    </div>
                </div>
            </div>
