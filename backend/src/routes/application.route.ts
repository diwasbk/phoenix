...
applicationRouter.post("/submit", optionalAuthMiddleware, schemaValidateMiddleware(applicationSchema), applicationController.submitApplication);
applicationRouter.get("/all", jwtAuthMiddleware, authorizeAdminMiddleware, applicationController.getAllApplications);
applicationRouter.get("/by-user-id/:userId", applicationController.getAllApplicationsByUserId);
